...
const getPurchasedShoes = async (req, res) => {
    try {
        ...
        if (req.user.role === 'admin') {
            const purchasedShoes = await Purchaseshoe.find({}).populate("type");
            ...
            const purchasedShoes = await Purchaseshoe.find({ customer: customer._id }).populate("type");
...
const updatePurchasedShoe = async (req, res) => {
    try {
        ...
        const updatedPurchasedShoe = await Purchaseshoe.findByIdAndUpdate(id, { type, price, purchaseDate }, { new: true });
...
const deletePurchasedShoe = async (req, res) => {
    try {
        ...
        const deletedPurchasedShoe = await Purchaseshoe.findByIdAndDelete(id);
